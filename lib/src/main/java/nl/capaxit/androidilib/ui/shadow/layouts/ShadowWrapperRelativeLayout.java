 ...
 */
public final class ShadowWrapperRelativeLayout extends RelativeLayout {
    ...
    public ShadowWrapperRelativeLayout(final Context context) {
        this(context, null);
    }

    public ShadowWrapperRelativeLayout(final Context context, final AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ShadowWrapperRelativeLayout(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        final TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.ShadowWrapperRelativeLayout);
        if (attributes != null) {
            shadowHeight = attributes.getDimensionPixelSize(R.styleable.ShadowWrapperRelativeLayout_capaxitShadowWrapperRelativeShadowHeight, -1);
            final int shadowSides = attributes.getInt(R.styleable.ShadowWrapperRelativeLayout_capaxitShadowWrapperRelativeShadowSide, -1);
//            Convert attribute flags to enumset. The values of the enum correspond to the values in the attrs.xml.
    ...
    private void drawCornerShadow(final Canvas canvas, final Corner corner, final float startAngle) {
        ...
        canvas.drawArc(corner.getCornerRect(getWidth(), getHeight(), shadowHeight), startAngle, (float) ShadowWrapperRelativeLayout.CORNER_SHADOW_SWEEP_ANGLE, true, cornerPaint);
    }
